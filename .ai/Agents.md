...
docs/current/prisma_principles_to_optmise.md
docs/current/features.md
```

If a required detail is missing from current documents, do not invent it.

Use the smallest safe implementation that satisfies Phase 1.
...
- proper error handling on backend APIs, frontend data flows, and user-triggered mutations
...
- handle errors explicitly at route, service, and UI boundaries; do not leave failed flows silent.
...
- update `docs/current/features.md` whenever new implemented features are added, changed, or completed.
